...
import { api, type ServiceStatus, type ServiceIncident, type StatusIndicator } from '../api/client';
...
export default function StatusView() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if(active) {
        if(page.status==='fulfilled') { setStatus(page.value);setError(null);setUpdated(Date.now()); }
        else setError(page.reason instanceof Error ? page.reason.message : String(page.reason));
    ...
    void load();
    const timer=setInterval(load,60000), tick=setInterval(()=>setNow(Date.now()),1000);
  ...
  const choose = useCallback((key:string)=>{setSelected(current=>current===key?null:key);setDetail(null);},[]);
  ...
  return <div className="status-page">
    <div className="page-heading"><div><span className="eyebrow">INFINIA · STATUS</span><h1>{t('status.title')}</h1></div>
      <div className="refresh-actions">{updated && <span className="refresh-time">{now-updated<5000?t('status.updatedJustNow'):t('status.updatedSecondsAgo',{n:Math.max(0,Math.floor((now-updated)/1000))})}</span>}<button className="btn btn-secondary" disabled={loading} onClick={()=>setRefresh(n=>n+1)}>{t('status.refresh')}</button></div></div>
    {error && <div role="alert" className="notice">{error} · {t('common.error')} <button className="btn btn-secondary" onClick={()=>setRefresh(n=>n+1)}>{t('common.retry')}</button></div>}
        ...
        {tab==='hive'? <div className="card hive-panel">
          ...
          <div className="hive-layout">
            <div className="hive-visual">
              <svg className="hive" viewBox={`${minX} ${minY} ${width} ${height}`} aria-label={t('status.hiveHint')}>
                {cells.map((_,index)=>{
                  ...
                  return <g key={component?.key ?? index} className={selected && component?.key!==selected && !isCenter?'comb dimmed':'comb'}>
                    ...
                    {isCenter ? <g className="hive-overall"><text x={x} y={y-20} textAnchor="middle" className="center-title">{t('status.overall')}</text><text x={x} y={y+12} textAnchor="middle" className={'center-status '+overall}>{indicatorText(overall)}</text><text x={x} y={y+42} textAnchor="middle" className="center-metric">{average==null?'—':average.toFixed(2)+'%'}</text></g> : component &&
                      <g>{component.history.slice(-90).map((day,i)=>{
                        ...
                        const label=`${componentText(component.key)} · ${day.date} · ${indicatorText(day.indicator)} · ${day.uptimePercent==null?'—':day.uptimePercent.toFixed(2)+'%'}`;
            ...
            <div className="service-list" data-testid="hive-legend">{components.map((c,index)=><button key={c.key} className={'service-row '+(selected===c.key?'active':'')} aria-pressed={selected===c.key} onClick={()=>choose(c.key)}>
              <span className="service-number">{(index+1).toString().padStart(2,'0')}</span><span className="service-name">{componentText(c.key)}<small data-testid="component-live-status"><i className={c.indicator}/>{indicatorText(c.indicator)}</small></span>
